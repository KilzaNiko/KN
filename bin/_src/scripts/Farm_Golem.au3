...
Func Farm_Golem($bParam = True, $aStats = Null)
    ...
    ;Runs, Dungeon Level, Guided Auto, Refill, Gold Goal, Target Boss
        ...
        Switch $sLocation
            ...
            Case "battle-boss"
                If $Farm_Golem_Target_Boss = True Then
					...
					If $Farm_Golem_Timer_Target_Boss = False Then
						If _Sleep($Delay_Target_Boss_Delay) Then ExitLoop
					EndIf
					clickPoint("395, 317")
					If $sLocation = "battle" Then clickBattle()
					;-----Time of focus Boss-----¬
					If $Farm_Golem_Timer_Target_Boss = True Then
						Local $cTimerFocus = TimerDiff($cTimer)
						Local $show = StringSplit($cTimerFocus, ".")
						MsgBox(0, "Time of Target Boss", "Target Boss Delay: " & $show[1])
						ExitLoop
					EndIf
					;----------------------------^
                EndIf
