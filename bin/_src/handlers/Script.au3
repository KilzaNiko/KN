...
Func Start_Schedule()
    If StringLeft($g_sScript, 4) = "Farm" Or StringLeft($g_sScript, 6) = "Attack" Then
        ...
        If $Expedition_Expedition_Script = True Then
            $aStructure = CreateArr(TimerInit(), $Expedition_Check_Intervals*60*60)
            $aAction = CreateArr("doExpedition()")

            Schedule_Add("_Expedition", $aAction, $SCHEDULE_TYPE_TIMER, "*", $aStructure, $SCHEDULE_FLAG_RunSafe, 0, True, False)
        EndIf
        ...
        If $Astroleague_Astroleague_Script = True Then
            $aStructure = CreateArr(TimerInit(), $Astroleague_Check_Intervals*60)
            $aAction = CreateArr('_Schedule_Astroleague()')
            Schedule_Add("_Astroleague", $aAction, $SCHEDULE_TYPE_TIMER, "*", $aStructure, $SCHEDULE_FLAG_RunSafe, 0, True, False)
        EndIf

        If $Champion_Champion_League_Script = True Then
            $aStructure = CreateArr(TimerInit(), $Champion_Check_Intervals*60)
            $aAction = CreateArr('_Schedule_Champion()')
            Schedule_Add("_Champion", $aAction, $SCHEDULE_TYPE_TIMER, "*", $aStructure, $SCHEDULE_FLAG_RunSafe, 0, True, False)
        EndIf
    EndIf
EndFunc
...
Func _Schedule_Stop()
    ...
    Schedule_RemoveByName("_Expedition")
    Schedule_RemoveByName("_Guardian")
    Schedule_RemoveByname("_Astroleague")
    Schedule_RemoveByname("_Champion")
EndFunc
...
Func _Schedule_Astroleague()
    Local $aValues = Stats_Values_GetSpecific(Stats_GetValues($g_aStats), CreateArr("PVP_Wins", "PVP_Runs"))
    Local $aParam = CreateArr(0, 0, $Astroleague_Attack_Strongest, $Astroleague_Attack_Random, $Astroleague_Random_Only, $Astroleague_Focus_Enemy, $Astroleague_Draw_Match)
    _RunScript("Attack_PVP", $aParam, $aValues)
EndFunc

Func _Schedule_Champion()
    Local $aValues = Stats_Values_GetSpecific(Stats_GetValues($g_aStats), CreateArr("Champion_Wins", "Champion_Runs"))
    Local $aParam = CreateArr(0, 0, $Champion_Attack_Strongest, $Champion_Focus_Enemy)
    _RunScript("Attack_Champion", $aParam, $aValues)
EndFunc
