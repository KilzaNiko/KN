...
;-----¬
;Farm Hero Dungeons func's
Func Hero($sText, $iLogType = $LOG_DEBUG)
    If $sText = $g_sStatus Then Return False
    Log_Add($sText, $iLogType)
    If isDeclared("Hero") = True Then $Hero = $sText

    $g_sStatus = $sText
    Return True
EndFunc

Func Level_Dungeon($sText, $iLogType = $LOG_DEBUG)
    If $sText = $g_sStatus Then Return False
    Log_Add($sText, $iLogType)
    If isDeclared("Level_Dungeon") = True Then $Level_Dungeon = $sText
